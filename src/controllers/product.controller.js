...
exports.createProduct = async (req, res) => {
    try {
        ...
        if (!newProduct) return res.status(400).json({ error: 'No fue posible crear la product' });

        return res.status(201).json({ datos: newProduct });
    } catch (error) {
        return res.status(500).json({
            message: 'Hubo un error al crear la product',
            error: error.message
        })
    }
}
...
exports.updateProductById = async (req, res) => {
    try {
        ...
        return res.status(500).json({
            message: 'Hubo un error al actualizar la product',
            error: error.message
        })
    }
}
...
exports.deleteProductById = async (req, res) => {
    try {
        ...
        return res.status(200).json({ message: 'La product se elimino correctamente' });
    } catch (error) {
        return res.status(500).json({
            message: 'Hubo un error al eliminar la product',
            error: error.message
        })
    } 
}